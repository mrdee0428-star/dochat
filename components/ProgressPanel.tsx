...
export default function ProgressPanel({ progress, stores }: Props) {
    ...
    return (
      <div className="bg-surface-100 border border-surface-300 rounded-2xl p-8 h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-zinc-600 text-xs mt-1">Kết quả sẽ hiển thị ở đây</p>
        </div>
      </div>
    );
  }
          ...
          return (
            <div
              ...
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all duration-300
                ${entry.status === 'done' ? 'bg-emerald-500/5 border border-emerald-500/20' : ''}
                ${entry.status === 'error' ? 'bg-red-500/5 border border-red-500/20' : ''}
                ${entry.status === 'scraping' ? 'bg-brand-500/5 border border-brand-500/20' : ''}
                ${entry.status === 'pending' ? 'bg-surface-200/50 border border-surface-300' : ''}
              `}
            >
              {/* Status icon */}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <span className={`truncate font-medium ${entry.status === 'pending' ? 'text-zinc-500' : 'text-zinc-300'}`}>
                    {store.name}
                  </span>
                </div>
              </div>
              ...
              <div className="flex-shrink-0 text-right">
                {entry.status === 'scraping' && (
                  <span className="text-xs text-brand-400 animate-pulse">Đang crawl...</span>
                )}
                {entry.status === 'done' && (
                ...
                {entry.status === 'pending' && (
                  <span className="text-xs text-zinc-600">Chờ...</span>
                )}
              </div>
